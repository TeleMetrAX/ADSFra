...
class DualBufferSystem:
    def __init__(self, dataset_name, window_size, ws_conn_uri, processing_func):
    ...
    async def ingest_data(self):
        ...
        async with websockets.connect(self.ws_conn_uri) as websocket:
            ...
            await websocket.send(json.dumps({"command": "SELECT_DATASET", "dataset_name": self.dataset_name}))
        ...
        while not self.processing_finished or self.buffer_state == BufferState.PROCESSING:
            ...
            async with self.lock:
                if self.buffer_state == BufferState.PROCESSING:
                    ...
                    await self.processing_func(np.array(self.processing_buffer['input_data']),
                                               np.array(self.processing_buffer['label']))
                    self.buffer_state = BufferState.FILLING
